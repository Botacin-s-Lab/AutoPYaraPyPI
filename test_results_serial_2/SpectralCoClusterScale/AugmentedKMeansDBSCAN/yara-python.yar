rule testrules
{
	...
	strings:
		...
		$x0 = { 74 02 FF D0 83 C6 04 3B F7 72 F1 5F 5E C3 56 57 BE 70 33 00 10 BF 70 33 00 10 EB 0B 8B 06 85 C0 } 
		//Benign FP est: -0.0 Malicious FP est: -0.0 Entropy: 3.5625 Found in 3 files
		$x3 = { 74 73 6C 74 67 64 77 66 74 73 78 74 66 64 77 65 74 70 01 74 6D 42 77 7F 74 70 01 74 67 52 77 65 } //This might be a string? Looks like:tsltgdwftsxtfdwetptmBwtptgRwe
		...
		$x5 = { 0C 73 4C 71 5F 41 60 7F 74 4D 06 76 7D 7C 65 7A 05 64 5E 76 7D 6C 7F 61 5C 41 70 7F 77 74 57 63 } //This might be a string? Looks like:sLq_A`tMv}|ezd^v}la\ApwtWc
		//Benign FP est: -0.0 Malicious FP est: -0.0 Entropy: 4.9375 Found in 1 files
		$x1 = { 61 7D 5E 7F 01 4C 44 74 70 50 43 06 7E 4D 5B 40 42 79 47 4D 46 5D 05 67 1E 57 51 02 03 7A 72 1A } //This might be a string? Looks like:a}^LDtpPC~M[@ByGMF]gWQzr
