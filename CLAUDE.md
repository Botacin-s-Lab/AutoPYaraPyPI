...
**Branch protection:** `main` is meant to reject direct pushes from everyone except the release job's bot token (via a GitHub Ruleset bypass entry — a repo setting, not something in this repo's files; see RELEASING.md's "Branch protection" section for the exact setup). If that Ruleset isn't configured yet, `release`'s `git push origin HEAD:main` step will start failing the moment it is — don't be surprised by that failure mode if protection gets tightened without also re-checking the bypass entry.
