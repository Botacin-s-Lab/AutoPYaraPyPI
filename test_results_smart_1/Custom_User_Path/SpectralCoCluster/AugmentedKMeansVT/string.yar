rule testrules
{
	...
	strings:
		...
		$x3 = { 7F 66 6C 5B 7E 76 5E 44 7E 4C 42 41 79 5C 0D 42 78 61 7C 4F 7B 71 60 07 7B 4F 52 00 7A 5F 46 0D 65 61 01 1A 64 72 73 5C 6C 07 67 59 6F 58 51 5A 54 62 45 47 57 72 04 40 57 06 77 4D 56 5B 7B 05 } //This might be a string? Looks like:fl[~v^D~LBAy\\rBxa|O{q`{ORz_F\readrs\lgYoXQZTbEGWr@WwMV[{
		...
		$x1 = { 53 76 7A 5C 5E 02 51 01 1E 47 4D 7D 0D 5D 66 6F 05 70 46 4C 78 4C 6C 42 72 0D 0D 5A 76 71 40 58 4F 73 65 40 7C 62 5A 5C 4F 61 07 52 52 44 5C 54 6D 43 56 43 77 4F 64 79 59 66 03 51 7F 7D 50 06 } //This might be a string? Looks like:Svz\^QGM}\r]fopFLxLlBr\r\rZvq@XOse@|bZ\OaRRD\TmCVCwOdyYfQ}P
		...
		$x7 = { 66 7F 7B 50 60 73 42 56 72 4D 45 6C 07 73 05 54 62 0C 40 79 58 73 42 56 76 7C 43 65 52 05 7E 7C 76 74 0D 51 7D 7F 04 56 06 67 7F 57 58 6F 43 7C 7D 5D 41 57 72 00 4F 65 66 7F 04 56 58 01 03 56 } //This might be a string? Looks like:f{P`sBVrMElsTb@yXsBVv|CeR~|vt\rQ}VgWXoC|}]AWrOefVXV
		//Benign FP est: -0.0 Malicious FP est: -0.0 Entropy: 5.382659765557392 Found in 2 files
		$x5 = { 47 50 01 40 57 44 02 43 79 07 1E 43 0D 71 77 42 46 65 70 4D 56 57 7D 4C 78 5B 7E 4C 0D 4F 7B 4F 46 1A 64 05 51 79 61 04 7B 6D 62 04 0C 5F 6F 07 41 43 56 06 51 02 53 01 7A 7D 5C 01 1E 61 59 00 } //This might be a string? Looks like:GP@WDCyC\rqwBFepMVW}Lx[~L\rO{OFdQya{mb_oACVQSz}\aY
		//Benign FP est: -0.0 Malicious FP est: -0.0 Entropy: 4.725798827786958 Found in 3 files
		$x9 = { 74 74 52 74 74 74 74 74 01 53 40 52 01 74 41 74 5B 7B 7C 57 52 77 61 78 05 5D 63 72 5D 45 56 4C 77 42 56 58 0C 5B 56 58 73 41 7C 72 7B 5D 57 58 00 43 51 76 77 5C 6F 66 77 4C 51 62 01 52 54 62 } //This might be a string? Looks like:ttRtttttS@RtAt[{|WRwax]cr]EVLwBVX[VXsA|r{]WXCQvw\ofwLQbRTb
		...
		condition:
(3 of ($x0,$x1,$x2) ) or (3 of ($x3,$x4,$x5) ) or (2 of ($x6,$x7) ) or (2 of ($x8,$x9) )}